		...
		<GenericForm
			...
			<div className='flex flex-col flex-wrap w-full gap-4 '>
				...
				{fields.map((item, index) => (
					<div
						key={item.id}
						className='w-fit flex flex-row flex-wrap items-center justify-around'>
						<input
							{...register(`items.${index}.itemName`, {
								required: 'El Articulo es requerido',
							})}
							className={`w-fit p-2 form-control ${
				...
				{errors.items && (
					<span className='text-red-700 fs-6'>{errors.items.message}</span>
				)}
			</div>
